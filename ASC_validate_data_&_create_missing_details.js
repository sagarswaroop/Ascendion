...
define(['N/record', 'N/file', "N/runtime", "N/search", "N/query"], (record, file, runtime, search, query) => {
    ...
    function getInputData() {
        try {
            ...
            if (deploymentID == "customdeploy_csv_file_vlaidate") {
                try {
                    ...
                    log.error({ title: "Get Input : customdeploy_val_data_add_miss_detail", details: error });
                }
            }

            if (deploymentID == "customdeploy_asc_validate_data_create_miss_det") {
                if (deploymentID == "customdeploy_error_rec_data_validate") {
                    try {
                        const validateDataList = [];
                        const fileId = scriptObj.getParameter({ name: "custscript_csv_fileid" });
                        log.debug({ title: "fileId", details: fileId });
                    ...
                    if (billList.length == 0) {
                        ...
                        if (VendorDetails.length > 0) {
                            VendorDetails.forEach(detail => {
                                ...
                                if (detail.startdate > updatedData.endDate) {
